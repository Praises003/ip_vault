...
export default function AssetCard({ asset }: AssetCardProps) {
  const plan = asset.licensePlans?.[0] || {}
      ...
      <Link href={`/marketplace/asset/${asset.id}`}>
        ...
        <CardContent className="pt-0">
          ...
          <div className="flex items-center justify-between text-xs text-gray-500">
            <span className="flex items-center"><Eye className="h-3 w-3 mr-1" />{views}</span>
            <span className="flex items-center"><Download className="h-3 w-3 mr-1" />{downloads}</span>
